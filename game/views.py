...
def game(request):
    ...
    for data_question, data_answer in zip(Question.objects.all().values(), Answer.objects.all().values()):
        current_question_with_answer.extend([{'user': Profile.objects.get(id=request.user.id), 'points': Profile.objects.get(id=request.user.id).points, 'question': data_question['question'], 'array_of_answers': data_answer['array_of_answers'], 'correct_answer': data_answer['correct_answer']}])
